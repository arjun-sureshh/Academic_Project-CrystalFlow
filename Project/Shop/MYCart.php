...
<html>
    ...
    <?php
        ...
        if (isset($_POST["btn_checkout"])) {
                ...
                $selC = "select * from tbl_booking where shop_id='" .$_SESSION["uid"]. "'and booking_status='0'";
                $rs = $Conn->query($selC);
                $row=$rs->fetch_assoc();
                 $_SESSION["bid"] = $row["booking_id"];
                
                $upQry1 = "update tbl_booking set booking_date=curdate(),booking_amount='".$amt."',booking_status='1' where shop_id='" .$_SESSION["uid"]. "'";
				$Conn->query($upQry1);
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id where b.shop_id='".$_SESSION["uid"]."' and booking_status='0' and cart_status=0";
            $res = $Conn->query($sel);
                while ($row=$res->fetch_assoc()) {
                    ...
                    if ($rowst=$resst->fetch_assoc()) {
                        ...
                        $curStock = $rowst["stock"] - $chk["quantity"];
            ?>

            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assets/Files/Company/CProduct/<?php echo $rowpr["product_image"] ?>"
                        />
                </div>
                ...
                <div class="product-quantity">
                    <input  alt="<?php echo $row["cart_id"]?>" type="number" value="<?php echo $row["cart_quantity"] ?>" min="4" max="<?php echo $curStock ?>"/>
                </div>
